package march.twotwo;

import java.util.Scanner;
import march.twentysec.Reverse;

public class _0322_middle extends Reverse {

	public _0322_middle(String str) {
		super(str);
	}

	public static void main(String[] args) {
//		1. Reverse라는 클래스를 만들고 메소드로 String reverseString(String str)를 만듭니다.
//		   매개변수로 받은 str을 거꾸로 만든 String을 리턴하도록 만드세요.
		Scanner sc = new Scanner(System.in);
		System.out.println("문자열을 입력해주세요.");
		String str = sc.nextLine();
		
		Reverse rv = new Reverse(str);
		
		rv.reverseString(str);

//		2. 사용자가 입력한 문자열에서 중복된 문자를 제거한 문자열을 출력하세요.

//		3. 사용자가 입력한 문자열에서 3의 배수 자리의 문자는 3으로 5의 배수 자리의 문자는 5로 변경하여 출력하세요.
		
	}
}