...
public class _02_printAlphabet {
	...
	public static void main(String[] args) throws IOException {
		...
		//대문자 A 유니코드 65 알파벳의 개수 26개, 소문자 a 유니코드 97 
		if(inputUni >= 65 && inputUni <= 90) {
			System.out.println("대문자입니다.");
			System.out.println((char)inputUni);
						
			//1. 입력된 문자가 대문자이면 입력된 문자부터 대문자 Z까지 출력
			int i;
			
			for(i = inputUni; i <= 90; i++) {
				System.out.print((char)i);
			}
			//2. 대문자 A부터 입력된 문자까지 출력
			for(i = 65; i <= inputUni; i++) {
				System.out.print((char)i);
			}
			...
			for(i = inputUni; i <= 122; i++) {
				//i <= 'z' 가능, char 2byte, int 4byte이기 때문에 자동 형변환 되어서 인식
				System.out.print((char)i);
			}
			//2. 소문자 a부터 입력된 문자까지 출력
			for(i = 97; i <= inputUni; i++) {
				System.out.print((char)i);
			}
		} else {
